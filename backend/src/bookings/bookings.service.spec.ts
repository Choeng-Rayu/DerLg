...
describe('BookingsService', () => {
  ...
  describe('createBooking', () => {
    ...
    it('should reject past travel dates', async () => {
      ...
      await expect(
        service.createBooking('user-1', pastDto as any),
    ...
    it('should reject dates more than 2 years in advance', async () => {
      ...
      await expect(
        service.createBooking('user-1', futureDto as any),
      ).rejects.toThrow(BadRequestException);
    });
      ...
      mockPrisma.$transaction.mockImplementation(async (fn: any) => {
        const txClient = {
          booking: {
            create: jest
              .fn()
              .mockResolvedValue({ id: 'b1', bookingRef: 'DLG-2026-0001' }),
          },
          trip: {
            findUnique: jest.fn().mockResolvedValue({ pricePerPersonUsd: 100 }),
          },
  ...
  describe('cancelBooking', () => {
    it('should throw NotFoundException when booking not found', async () => {
      ...
      await expect(service.cancelBooking('b-999', 'user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw BadRequestException when trip already started', async () => {
      ...
      await expect(service.cancelBooking('b-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
